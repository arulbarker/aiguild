...
describe('computeNewExpiry', () => {
  ...
  it('baru/expired → now + 365 hari', () => {
    expect(computeNewExpiry(null, now).getTime()).toBe(now.getTime() + 365 * DAY)
    const past = new Date('2026-05-01T00:00:00Z')
    expect(computeNewExpiry(past, now).getTime()).toBe(now.getTime() + 365 * DAY)
  })

  it('masih aktif → numpuk dari tanggal habis lama', () => {
    const future = new Date('2026-07-01T00:00:00Z')
    expect(computeNewExpiry(future, now).getTime()).toBe(future.getTime() + 365 * DAY)
  })
})
