...
import { isAiGuildProduct, extractEmail, extractOrderId, extractAmount, isValidMayarToken, matchCourseByProduct } from './mayar-webhook'

describe('isValidMayarToken', () => {
  it('Authorization: Bearer <token> cocok', () =>
    expect(isValidMayarToken({ authorization: 'Bearer tok_abc' }, 'tok_abc')).toBe(true))
  it('token mentah di x-webhook-token cocok', () =>
    expect(isValidMayarToken({ 'x-webhook-token': 'tok_abc' }, 'tok_abc')).toBe(true))
  it('x-gateway-token (suntikan gateway) cocok', () =>
    expect(isValidMayarToken({ 'x-gateway-token': 'tok_abc' }, 'tok_abc')).toBe(true))
  it('token salah → false', () =>
    expect(isValidMayarToken({ authorization: 'Bearer salah' }, 'tok_abc')).toBe(false))
  it('tanpa expected token → false (fail-closed)', () =>
    expect(isValidMayarToken({ authorization: 'Bearer tok_abc' }, '')).toBe(false))
  it('header kosong → false', () =>
    expect(isValidMayarToken({}, 'tok_abc')).toBe(false))
})
...
describe('matchCourseByProduct', () => {
  ...
  it('cocok by productId', () =>
    expect(matchCourseByProduct(courses, { data: { productId: 'prod_web' } })?.id).toBe('c2'))
  it('produk tak dikenal → null (fail-closed)', () =>
    expect(matchCourseByProduct(courses, { data: { productId: 'prod_x' } })).toBe(null))
  it('course tanpa mayarProductId diabaikan', () =>
    expect(matchCourseByProduct([{ id: 'c3', mayarProductId: null }], { data: { productId: null } })).toBe(null))
})
