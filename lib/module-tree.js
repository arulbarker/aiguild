...
// Tentukan badge "BARU" / "UPDATE" untuk satu modul.
// BARU menang: kalau modul baru dibuat, tidak perlu sekaligus tampil UPDATE.
export function moduleBadge(mod, now = Date.now()) {
  const created = mod.createdAt ? new Date(mod.createdAt).getTime() : null
  if (created && now - created < BADGE_WINDOW_MS) return 'baru'

  const updated = mod.contentUpdatedAt ? new Date(mod.contentUpdatedAt).getTime() : null
  if (updated && now - updated < BADGE_WINDOW_MS) return 'update'
